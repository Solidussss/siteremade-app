...
async function context(req){if(!db||!anon)return null;const c=cookies(req),token=c.sr_access;if(!token)return null;const user=(await anon.auth.getUser(token)).data?.user;if(!user)return null;const p=(await db.from('profiles').select('role').eq('id',user.id).maybeSingle()).data;if(!p)return null;let ws=[];if(p.role==='owner')ws=(await db.from('workspaces').select('id').order('created_at')).data||[];else ws=((await db.from('workspace_members').select('workspace_id').eq('user_id',user.id)).data||[]).map(x=>({id:x.workspace_id}));if(!ws.length)return null;let wid=req.headers['x-workspace-id']||c.sr_workspace||ws[0].id;if(!ws.some(x=>x.id===wid))wid=ws[0].id;return{user,wid}}
