...
http.createServer = function patchedCreateServer(listener) { return originalCreateServer(async (req, res) => { try { const u = new URL(req.url, `http://${req.headers.host || 'localhost'}`); if (req.method === 'GET' && (u.pathname === '/' || u.pathname === '/index.html')) { const body = enhancedHtml('index.html');
    // Performance review: index.html is served here directly (not through
    // server.js's serve(), which already gzips everything else it hands
    // out — see that file), so it was the one thing on the login page
    // still going out uncompressed. Same technique, same reasoning: gzip
    // when the client says it accepts it, byte-identical once decoded.
    if (/\bgzip\b/.test(req.headers['accept-encoding'] || '')) { res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8', 'Content-Encoding': 'gzip', 'Vary': 'Accept-Encoding', 'Cache-Control': 'no-store' }); return zlib.gzip(body, (err, buf) => { if (err) { res.writeHead(500); return res.end(); } res.end(buf); }); }
