...
class Router {
  ...
  add(method, pattern, opts, handler) {
    ...
    // A production-readiness review found that a duplicate method+pattern
    // registration would previously fail silently: find() does a linear
    // scan and returns the first match, so the earlier-registered route
    // would win and the later one would just never run, with no error —
    // exactly the "which file wins" trap this router replaced. There are
    // no collisions among the routes registered today (verified), but this
    // makes a future one a startup crash instead of a silent shadow.
    if (this.routes.some(r => r.method === method && r.pattern === pattern)) {
      throw new Error(`[router] duplicate route registration: ${method} ${pattern}`);
    }
    const { regex, paramNames } = compilePattern(pattern);
    this.routes.push({ method, pattern, regex, paramNames, auth: opts.auth || 'none', handler });
    return this;
  }
