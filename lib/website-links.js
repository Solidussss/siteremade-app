...
//
// Phase 9 (multi-project, V54-WEBSITE-LINKS-MULTI-PROJECT-MIGRATION.sql):
// a workspace may now hold MORE THAN ONE row here -- one per purchased
// SiteRemade project the customer has connected. `workspace_id` is no
// longer the primary key (a surrogate `id` is); it keeps its FK to
// workspaces(id) and gained a plain (non-unique) index instead. The
// invariant this file always relied on for identity is unchanged and still
// enforced by a real unique index: `generator_project_id` still belongs to
// AT MOST ONE workspace, full stop -- findWorkspaceForProject below (used
// by public form-submission routing) needed zero changes for that reason.
//
// What this means for the functions below:
//   - getLinkForWorkspace(wid) is now explicitly a "one representative link"
//     helper (most recently linked), kept for the few callers that only
//     ever cared about a single default project (the canonical-bridge
//     recordBridgeSummary path). New, genuinely multi-project-aware code
//     uses listLinksForWorkspace / getLinkForWorkspaceAndProject instead.
//   - connectWorkspaceToProject no longer refuses because the workspace
//     already has A link -- only because THIS project is already linked
//     (to this workspace: idempotent success; to a different one: refused).
//     This is the one behavior change that actually enables multi-project:
//     the same Phase 8 "Connect a website" picker, called again, now adds a
//     second (third, ...) project instead of being told the workspace is
//     already linked.
//   - the mismatch/candidate-review flow (Phase 6) stops being able to
//     CREATE new mismatches: "the canonical project isn't the one already
//     linked" is no longer an error state once a workspace can hold several
//     links -- it just means that particular project isn't connected yet,
//     same as never having connected anything (status 'not_linked', same as
//     before Phase 6 existed). Existing mismatch rows already sitting in
//     production from before this migration are untouched and remain
//     reviewable exactly as before via relinkWorkspace/Admin -- nothing
//     here deletes or auto-resolves a legacy mismatch.
'use strict';
...
// Phase 9: a workspace can have several rows now, so this can no longer use
// .maybeSingle() (which THROWS if more than one row matches -- it would
// break every multi-project workspace the moment it got a second link).
// Kept for callers that only ever need "a" link (recordBridgeSummary's
// legacy-mismatch bookkeeping); returns the most recently linked row.
// Anything that needs to reason about ALL of a workspace's projects must
// use listLinksForWorkspace or getLinkForWorkspaceAndProject instead.
async function getLinkForWorkspace(wid) {
  if (!db || !wid) return null;
  const { data, error } = await db.from(TABLE).select('*').eq('workspace_id', wid).order('linked_at', { ascending: false }).limit(1);
  if (error) throw error;
  return Array.isArray(data) && data[0] ? data[0] : null;
}

// Phase 9: every project linked to this workspace, most recently linked
// first -- the real multi-project read (Workplace project list, Admin).
async function listLinksForWorkspace(wid) {
  if (!db || !wid) return [];
  const { data, error } = await db.from(TABLE).select('*').eq('workspace_id', wid).order('linked_at', { ascending: false });
  if (error) throw error;
  return Array.isArray(data) ? data : [];
}

// Phase 9: the link for one specific (workspace, project) pair, if any --
// used wherever a caller already knows which project it's asking about
// (the canonical-bridge route, a project-scoped Workplace page) rather than
// "whichever link comes back first".
async function getLinkForWorkspaceAndProject(wid, projectId) {
  if (!db || !wid || !PROJECT_ID_RE.test(String(projectId || ''))) return null;
  const { data, error } = await db.from(TABLE).select('*').eq('workspace_id', wid).eq('generator_project_id', projectId).maybeSingle();
  if (error) throw error;
  return data || null;
}

// Phase 9: the (at most one, by construction -- see header) link for this
// workspace that's sitting in a legacy "needs review" mismatch state.
// mismatch_project_id can only ever have been set by pre-Phase-9 code (see
// recordBridgeSummary below), and pre-Phase-9 a workspace could hold only
// one row total, so at most one of a workspace's current rows can have it
// set -- .maybeSingle() is safe, not just convenient, here.
async function getMismatchedLinkForWorkspace(wid) {
  if (!db || !wid) return null;
  const { data, error } = await db.from(TABLE).select('*').eq('workspace_id', wid).not('mismatch_project_id', 'is', null).maybeSingle();
  if (error) throw error;
  return data || null;
}
...
async function recordBridgeSummary(wid, summary) {
  ...
  // Phase 9: looked up by (workspace, THIS canonical project) directly, not
  // "whichever link this workspace happens to have" -- a workspace may now
  // legitimately have other, unrelated projects linked too, and those must
  // never be touched or reinterpreted as a "mismatch" by a visit to the
  // single-project canonical bridge route.
  const existing = await getLinkForWorkspaceAndProject(wid, projectId);

  // Phase 8: a workspace with no link YET FOR THIS PROJECT is no longer
  // auto-linked here. Silently linking to whatever the builder resolves as
  // "canonical" (its own most-recently-purchased-project guess) is exactly
  // the invisible connection the Website page's "Connect a website" flow
  // replaces: the customer sees their own verified purchases (GET
  // /api/app/website/candidates) and explicitly chooses one (POST
  // /api/app/website/connect -> connectWorkspaceToProject below).
  //
  // Phase 9: this is also now the ONLY path for a not-yet-linked canonical
  // project, including one that differs from a project this workspace has
  // linked before -- "the builder now reports a different purchased
  // project" is no longer a data-integrity problem once a workspace can
  // hold several links; it's just a project that isn't connected yet, same
  // as the very first purchase ever was. It stops being possible for this
  // function to CREATE a new mismatch_project_id going forward. A workspace
  // whose ONLY existing link is already flagged mismatch from BEFORE this
  // migration keeps that flag untouched here (Admin still resolves it via
  // relinkWorkspace) -- this function only ever reads/writes the row that
  // matches THIS specific project id, never that unrelated legacy row.
...
// Phase 8/9: the explicit "connect a website" action (routes/website-
// bridge.js's POST /api/app/website/connect). The candidate passed in must
// come from a call the route just made to the builder with THIS SAME
// request's own access token (GET /api/app-bridge/website/candidates) --
// never anything the browser remembered from an earlier response, so
// ownership is re-verified at the moment of connecting, not just at the
// moment of listing.
//
// Phase 9: no longer refuses because the workspace already has A link --
// only because THIS SPECIFIC project is already linked. Linked to this
// same workspace already -> idempotent success (created:false). Linked to
// a DIFFERENT workspace -> refused (a project still belongs to at most one
// workspace, enforced by the real unique index, not just this check). This
// is what makes "one customer, several purchased SiteRemade projects" a
// normal, repeatable customer action instead of a staff relink: the same
// picker (GET /api/app/website/candidates), called again for a second
// purchase, now succeeds instead of getting `already_linked`.
async function connectWorkspaceToProject(wid, candidate) {
  ...
  const sameLink = await getLinkForWorkspaceAndProject(wid, candidate.projectId);
  if (sameLink) return { ok: true, link: sameLink, created: false };
  ...
  if (/duplicate key|unique/i.test(String(ins.error && ins.error.message || ''))) {
    // Lost a race: either this same (workspace, project) pair got linked a
    // moment ago (two tabs connecting at once) or the project itself did
    // (a different workspace won it first).
    const again = await getLinkForWorkspaceAndProject(wid, candidate.projectId);
...
async function relinkWorkspace(wid, requestedProjectId, { actorUserId } = {}) {
  ...
  // Phase 9: a workspace may hold several links now, but mismatch_project_id
  // can only ever have been set by pre-Phase-9 code (recordBridgeSummary no
  // longer creates new ones -- see its own comment), so exactly the ONE
  // link that's still flagged from before this migration is what a pending
  // review is about. Other, unrelated projects this workspace has since
  // connected are never touched by this function.
  const existing = await getMismatchedLinkForWorkspace(wid);
  if (!existing) return fail(409, 'no_review_pending', 'This workspace has no link waiting for review. Refresh Admin.');
...
module.exports = {
  ...
  listLinksForWorkspace, getLinkForWorkspaceAndProject, getMismatchedLinkForWorkspace,
};
