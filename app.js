...
function renderAdminWebsite(d){const host=qs('#adminWebsiteList');if(!host)return;const short=id=>{const s=String(id||'');return s.length>14?`${s.slice(0,9)}…${s.slice(-4)}`:s;};
  host.innerHTML=d.websiteLinksAvailable===false?'<p class="helper-copy">Website links aren’t available yet — the V52 migration hasn’t been applied.</p>':d.workspaces.map(w=>{const x=w.website||{linked:false};const detail=x.linked?`${esc(short(x.projectId))}${x.lastSeenRevision!==null?` · version ${esc(String(x.lastSeenRevision))}`:''}${x.linkedAt?` · linked ${esc(dateLabel(x.linkedAt))}`:''} · ${x.analyticsReady?'Analytics site ready':'Analytics site not set up yet'}${x.needsReview?` · builder now reports ${esc(short(x.reportedProjectId))} — link left unchanged`:''}`:'Links itself the first time the customer opens their Website view with a purchased builder project.';return `<div class="admin-row"><div><strong>${esc(w.businessName)}</strong><span>${detail}</span></div><div class="admin-actions"><span class="status-pill ${x.linked&&!x.needsReview?'':'neutral'}">${x.linked?(x.needsReview?'REVIEW':'LINKED'):'NOT LINKED'}</span></div></div>`;}).join('');
