...
function install(){if(installed||typeof state==='undefined'||typeof renderConversationWindow!=='function')return setTimeout(install,100);installed=true;patchConversation();ensure();load();document.addEventListener('click',e=>{if(e.target.closest('[data-view="inbox"]'))setTimeout(()=>{load();if(status?.connected)sync()},120)});const q=new URLSearchParams(location.search);if(q.get('mailbox')){history.replaceState({},'',location.pathname);setTimeout(load,200)}setInterval(()=>{const v=$('#view-inbox');if(v?.classList.contains('active')&&status?.connected)sync()},60000)}
