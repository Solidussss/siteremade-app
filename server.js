...
// TEMP auth diagnostic: verify the configured owner credential against Supabase without logging secrets.
setTimeout(async()=>{try{if(process.env.OWNER_EMAIL&&process.env.OWNER_PASSWORD&&anon){const r=await anon.auth.signInWithPassword({email:process.env.OWNER_EMAIL,password:process.env.OWNER_PASSWORD});console.log('Owner auth diagnostic:',r.error?{ok:false,message:r.error.message,code:r.error.code||'',status:r.error.status||''}:{ok:!!r.data?.session});if(r.data?.session)await anon.auth.signOut();}}catch(e){console.error('Owner auth diagnostic error:',e?.message||e)}},1500).unref();
