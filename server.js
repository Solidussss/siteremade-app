...
// TEMP owner-password identity check using only SHA-256 fingerprints; no password is logged or stored in source.
setTimeout(()=>{try{const h=crypto.createHash('sha256').update(String(process.env.OWNER_PASSWORD||'')).digest('hex');const which=h==='534cd70c262b8ff771b307f738fa6ea41b713e4982891d0ea0c6fc226f69e810'?'candidate-1':h==='829cd3f681c1d7d4e3867c4a834186345d1f13a3f8ab0110b446cbab7ebdd9ea'?'candidate-2':'neither';console.log('Owner password fingerprint match:',which)}catch(e){console.error('Owner password fingerprint check failed')}},1200).unref();
