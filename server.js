...
return json(res,201,{ok:true,lead:mapLead(l)});
  ...
  if(m==='GET'&&p==='/api/app/admin'){if(!c.owner)return json(res,403,{ok:false,message:'Owner only.'});const [profiles,members,auditRows,allLeads,allSpend,allFunds,linkList,builderBridge]=await Promise.all([q(db.from('profiles').select('*').order('created_at',{ascending:true})),q(db.from('workspace_members').select('*')),q(db.from('audit_logs').select('*').order('created_at',{ascending:false}).limit(50)),q(db.from('leads').select('workspace_id')),q(db.from('ad_spend').select('workspace_id,spend')),q(db.from('ad_funds').select('workspace_id,amount,status')),websiteLinks.listLinks(),generatorBridge.probeBridge()]);const websiteFor=wid=>{const l=linkList.rows.find(x=>x.workspace_id===wid);if(!l)return {linked:false};const needsReview=!!l.mismatch_project_id,cands=needsReview?websiteLinks.sanitizeCandidates(l.mismatch_candidates):null;return {linked:true,projectId:l.generator_project_id,lastSeenRevision:Number.isInteger(l.last_seen_revision)?l.last_seen_revision:null,linkedAt:l.linked_at||null,updatedAt:l.updated_at||null,analyticsReady:!!l.analytics_site_id,needsReview,reportedProjectId:l.mismatch_project_id||null,reportedAt:l.mismatch_seen_at||null,candidatesCapturedAt:cands?(l.mismatch_candidates_at||null):null,candidates:cands?cands.map(x=>({projectId:x.projectId,purchasedAt:x.purchasedAt,revision:x.revision,current:x.projectId===l.generator_project_id,reported:x.projectId===l.mismatch_project_id})):null};};const workspaces=c.workspaces.map(w=>({...mapWorkspace(w),leads:allLeads.filter(x=>x.workspace_id===w.id).length,adSpent:allSpend.filter(x=>x.workspace_id===w.id).reduce((s,x)=>s+Number(x.spend||0),0),adFunded:allFunds.filter(x=>x.workspace_id===w.id&&x.status==='Funded').reduce((s,x)=>s+Number(x.amount||0),0),website:websiteFor(w.id)}));return json(res,200,{ok:true,workspaces,websiteLinksAvailable:linkList.available,websiteLinkCandidatesAvailable:linkList.candidatesAvailable,builderBridge,contactIntake:publicLimits.describe(),users:profiles.map(p=>({id:p.id,name:p.name,role:p.role,workspaceIds:members.filter(m=>m.user_id===p.id).map(m=>m.workspace_id)})),audit:auditRows});}
