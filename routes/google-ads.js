...
// Phase 8: the Google Ads API refuses every request that doesn't carry a
// `developer-token` header ("A developer token is required in the
// developer-token HTTP header for every API call" -- developers.google.com/
// google-ads/api/rest/auth). adsFetch() never sent one, so once a workspace
// was OAuth-connected, account listing and campaign reads could only ever
// fail, while the connection still showed as "Connected". The token is the
// GOOGLE_ADS_DEVELOPER_TOKEN already listed in .env.example and already used
// for server.js's integrations.googleAds flag. Read per call (not captured
// at load) so the status/`configured` answer always matches what a request
// would actually send.
...
module.exports = function registerGoogleAdsRoutes(router) {
  ...
  router.get('/api/app/google-ads/status', { auth: 'user' }, async (req, res, { c, json }) => {
    ...
    return json(res, 200, { ok: true, configured: !!(googleId && googleSecret && managerId && developerToken()), connected: !!row, scopeReady, email: row?.google_user_email || '', managerCustomerId: managerId, selectedCustomerId: row?.selected_customer_id || '', apiReady: !!row && scopeReady, status: pub?.status || (!row ? 'not_connected' : scopeReady ? 'connected' : 'scope_missing'), redirectUri });
  });
