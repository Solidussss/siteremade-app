(()=>{
let installed=false,smsSyncing=false;
...
async function syncSms(){if(smsSyncing)return;smsSyncing=true;const b=$('#v38SmsSync');if(b){b.disabled=true;b.textContent='Syncing…'}try{const r=await post('/api/app/integrations/twilio/sync');if(typeof refreshLight==='function')await refreshLight();if(typeof renderInbox==='function')renderInbox();decorateInbox();if(b)b.textContent=r.added?`Synced ${r.added}`:'Up to date';setTimeout(()=>decorateInbox(),1400)}catch(e){alert(e.message);decorateInbox()}finally{smsSyncing=false}}
